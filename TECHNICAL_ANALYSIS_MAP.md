...
> API эндпоинты см. в `server/src/api/routes/map.ts`. Упоминания Convex в документе устарели.

## Обзор архитектуры

Система карты построена на базе **Mapbox GL JS** с использованием React и Elysia API. Архитектура следует принципам **Feature-Sliced Design (FSD)** и разделена на несколько слоёв:
