import type { Episode, ResonanceArchetype, SceneNode, VoteOption } from '../types/resonance'
...
const scenes: SceneNode[] = [
  {
    ...
    allowKudos: false,
    checks: [
      { skill: 'perception', dc: 11, positionOptimum: 3, onSuccess: { grantFlag: 'noticed_listener' } }
    ]
  },
  {
    id: 'entry_vote',
    ...
    timerMs: 15000,
    options: vote([
      { id: 'storm', text: 'Штурм: врыв на скорости (+инициатива, +тревога)', nextScene: 'hard_landing', weightMultipliers: { guardian: 1.5 }, rewards: { alertDelta: 2 } },
      {
        id: 'stealth',
        ...
        checks: [{ skill: 'stealth', dc: 12, positionOptimum: 3, onFail: { alertDelta: 1, strainDelta: 2 } }]
      },
      { id: 'lenses', text: 'Активировать Линзы: увидеть ловушки', nextScene: 'hard_landing', weightMultipliers: { visionary: 1.5, empath: 1.2 }, rewards: { alertDelta: 1 } },
    ]),
    allowBrake: true,
  },
  {
    ...
    rewards: { flagsAdd: ['medic_disoriented'], strainDelta: 2 }
  },
  {
    id: 'statue_decision',
    kind: 'vote',
    ...
    options: vote([
      {
        ...
        checks: [{ skill: 'authority', dc: 12, positionOptimum: 1, onSuccess: { alertDelta: 1 }, onFail: { alertDelta: 2, strainDelta: 3 } }]
      },
      {
        id: 'hold',
        ...
        checks: [{ skill: 'insight', dc: 11, positionOptimum: 3, onSuccess: { grantFlag: 'surprise_round' }, onFail: { alertDelta: 1 } }]
      },
    ]),
    allowInterrupt: true,
  },
  {
    ...
    next: 'aftermath',
    checks: [
      { skill: 'authority', dc: 13, positionOptimum: 1, onSuccess: { strainDelta: -2 }, onFail: { strainDelta: 4 } },
  ...
  },
  {
    ...
    options: vote([
      { id: 'encrypt', text: 'Скачать и зашифровать — только для нас', nextScene: 'parley', rewards: { items: [{ id: 'data_copy', qty: 1 }], trustDelta: -3, strainDelta: 4, flagsAdd: ['has_copy'] } },
      { id: 'sanitize', text: 'Удалить поврежденное — скрыть правду', nextScene: 'parley', rewards: { trustDelta: -1, strainDelta: 1, flagsAdd: ['sanitized'] } },
      { id: 'deliver', text: 'Взять как есть — по инструкции', nextScene: 'parley', rewards: { trustDelta: 2, strainDelta: 0 } },
    ]),
    allowKudos: true,
  },
  {
    ...
    options: vote([
      { id: 'fight', text: 'Стрелять', nextScene: 'debrief', weightMultipliers: { guardian: 1.3 }, rewards: { alertDelta: 2, strainDelta: 3, trustDelta: -2 } },
      { id: 'peace', text: 'Мир — попытаться договориться', nextScene: 'debrief', weightMultipliers: { empath: 1.3 }, checks: [{ skill: 'empathy', dc: 13, positionOptimum: 2, onSuccess: { alertDelta: -1 }, onFail: { strainDelta: 2 } }] },
      { id: 'wait', text: 'Ждать и сканировать', nextScene: 'debrief', weightMultipliers: { visionary: 1.3, skeptic: 1.1 }, checks: [{ skill: 'insight', dc: 12, positionOptimum: 3, onSuccess: { grantFlag: 'lever_spotted' }, onFail: { alertDelta: 1 } }] },
    ]),
    allowInterrupt: true,
  ...
  },
  {
    ...
    options: vote([
      { id: 'truth', text: 'Сказать правду о выстреле в спину', nextScene: 'complete', rewards: { trustDelta: 3, strainDelta: -2 } },
      { id: 'blackmail', text: 'Шантаж: окно 28 минут, двойная оплата', nextScene: 'complete', weightMultipliers: { skeptic: 1.2, guardian: 1.1 }, rewards: { trustDelta: -2, strainDelta: 2, items: [{ id: 'bonus_pay', qty: 1 }] }, checks: [{ skill: 'rhetoric', dc: 13, positionOptimum: 2, onSuccess: { trustDelta: 1 }, onFail: { strainDelta: 2 } }] },
      { id: 'report', text: 'Следовать протоколу', nextScene: 'complete', weightMultipliers: { empath: 1.1 }, rewards: { trustDelta: 1, strainDelta: 0 } },
    ]),
    allowKudos: true,
  },
  {
