...
import { SurvivalPlayer, HexCell, HexCoordinate, ResourceType } from '../types';
...
export const HUD: React.FC<HUDProps> = ({ player, turn, selectedHex, mapData, visibleHexes, onMove, onEndTurn }) => {
  ...
  const moveCost = dist; // Simplistic 1:1 cost for demo
  
  const isVisible = selectedHex ? visibleHexes.has(hexToString(selectedHex)) : false;

  const getResourceLabel = (type: ResourceType) => {
    switch (type) {
      case ResourceType.SCRAP: return <span className="text-gray-400">SCRAP HEAP</span>;
      case ResourceType.FOOD: return <span className="text-green-400">GAME TRAIL</span>;
      case ResourceType.WATER: return <span className="text-blue-400">WATER SOURCE</span>;
      case ResourceType.FUEL: return <span className="text-orange-400">FUEL DEPOT</span>;
      case ResourceType.TECH: return <span className="text-purple-400">TECH CACHE</span>;
  ...
  return (
    <>
        ...
        <div className="bg-terminal-black/80 border border-hex-border backdrop-blur-sm p-4 rounded text-terminal-green pointer-events-auto">
          ...
          <div className="flex gap-6 text-sm">
            ...
            <div>
              ...
              <span className="text-lg">{player.position.q}, {player.position.r}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-terminal-black/90 border-l-4 border-terminal-green p-4 backdrop-blur-sm shadow-lg shadow-green-900/10">
          ...
          <div className="mb-4">
            <div className="flex justify-between text-xs mb-1">
              ...
              <span className="text-white">{player.ap} / {player.maxAp}</span>
            </div>
      ...
      <div className="absolute bottom-6 right-6 flex flex-col gap-4 items-end pointer-events-auto">
        ...
        {/* Context Panel for Selected Hex */}
        {selectedCell && (
          <div className="bg-terminal-black/90 border border-hex-border p-4 w-64 backdrop-blur-sm mb-2 text-right">
             ...
             {isVisible ? (
               <>
                 ...
                 <div className="text-xs text-gray-400 mb-2">
                   THREAT: <span className={selectedCell.threatLevel === 'SAFE' ? 'text-green-400' : 'text-red-400'}>{selectedCell.threatLevel}</span>
                 </div>
               </>
             ) : (
             ...
             <button 
               ...
               className={`w-full py-2 px-4 text-sm font-bold border transition-colors
                 ${player.ap >= moveCost 
